...
module Domain =
    ...
    type Column =
        ...
        | G
        | H
        static member List = [ A; B; C; D; E; F; G; H ]
    ...
    type ProposedMoveWithKnownPiece =
        { SelectedPiece: Piece
          From: Square
          To: Square }
    ...
    type GameState =
        { Board: Board
          ...
          Status: GameStatus
          Message: string
          MoveHistory: ValidatedMove list }
    ...
    type Vector =
        ...
        static member MakeUnit distance =
            ...
            { X = normalise distance.X
              Y = normalise distance.Y }
        ...
        static member ToSquare vector =
            ...
            if colIdx < Column.List.Length
               && colIdx >= 0
               && rowIdx < Column.List.Length
               && rowIdx >= 0 then
                Some(Column.List.[colIdx], Row.List.[rowIdx])
            else
                None
