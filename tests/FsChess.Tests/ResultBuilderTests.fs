...
module ResultBuilderTests =
    ...
    let result2 str : Result<string, MyErr> =
        match str with
        | "pass" -> Ok "This test passes"
        | _ -> Error Err1

    let resultGoesBoom () : Result<string, string> = failwith "Boom!"

    let resultToCompute () : Result<string, string> = Ok "pass"

    [<Tests>]
    let resultBuilderTests =
        testList
            "ResultBuilder"
            [ testCase "Works as expected - success"
              <| fun _ ->
              ...
              testCase "Works as expected - fail"
              <| fun _ ->
                  let aa =
                      result {
                          let! a = result1 "fail"
              ...
              testCase "Works as expected - binding"
              <| fun _ ->
                  let aa =
                      result {
              ...
              testCase "Works as expected - Zero"
              <| fun _ -> Expect.equal (result.Zero()) None "Should match"

              testCase "Works as expected - Combine"
              <| fun _ ->
                  let a = "pass" |> result1 >>= result2
              ...
              testCase "Works as expected - Bind"
              <| fun _ ->
                  let err = Err1
              ...
              testCase "Works as expected with exception - TryFinally"
              <| fun _ ->
